...
with tab9:
    ...
    if st.button(CUI["generate_button"], key="client_report_generate_btn"):
        ...
        concern_flags = {
            ...
            "compensation": (lumbar_compensation > 10) or (pelvic_compensation > 10) or (pelvic_rotation_rom > 10),
        ...
        if concern_flags["compensation"]:
            if client_lang_code == "ja":
                concern_items.append((
                    ...
                    f"しゃがむ・立ち上がる動きの中で、腰や骨盤が大きく動く場面が見られました"
                    f"(腰の反り {lumbar_compensation:.1f}°、骨盤の傾き {pelvic_compensation:.1f}°、"
                    f"骨盤の回旋 {pelvic_rotation_rom:.1f}°)。この状態が続くと、腰まわりへの負担が"
                    "蓄積しやすくなります。"
                ))
            else:
                concern_items.append((
                    "Lower back / pelvis compensation",
                    f"Noticeable lumbar and pelvic movement was observed during the squat "
                    f"(lumbar extension {lumbar_compensation:.1f}°, pelvic tilt {pelvic_compensation:.1f}°, "
                    f"pelvic rotation {pelvic_rotation_rom:.1f}°). Over time this can add strain around the lower back."
                ))
        ...
        if concern_flags["stability"]:
            if client_lang_code == "ja":
                concern_items.append((
                    "動作中のグラつき",
                    "しゃがむ・立ち上がる動作の中で、骨盤が横に揺れる場面がありました。"
                    "体幹やお尻まわりの筋力を使って、じっと支える意識をすると安定しやすくなります。"
                ))
            else:
                concern_items.append((
                    "Movement wobble",
                    "Some side-to-side pelvic movement was observed during the squat. "
        ...
        action_pool_en = [
            ...
            ("range", "Daily x5", "Slow, pain-free deep squat practice",
        ...
        phase_titles_en = ["① Standing", "② Descending", "③ Bottom", "④ Ascending"]
        phase_colors = [
            "#2E7D32" if not phase_std_flag["Standing"] else "#B8860B",
            "#C62828" if lumbar_compensation > 10 else "#2E7D32",
            "#B8860B" if phase_std_flag["Bottom"] else "#2E7D32",
            "#B8860B" if len(asym_flags) > 0 else "#2E7D32",
        ]
        phase_boxes = list(zip(phase_titles_en, phase_colors))
 
        if client_lang_code == "ja":
            phase_titles_local = ["① 立つ", "② しゃがむ", "③ 最下点", "④ 立ち上がる"]
            phase_notes_local = [
                "立位は安定しています" if not phase_std_flag["Standing"] else "立位でやや不安定な様子がありました",
                "腰が反りやすい傾向があります" if lumbar_compensation > 10 else "スムーズにしゃがめています",
                "最下点でグラつきが見られました" if phase_std_flag["Bottom"] else "最下点は安定しています",
                "左右差が見られました" if len(asym_flags) > 0 else "安定して立ち上がれています",
            ]
        else:
            phase_titles_local = phase_titles_en
            phase_notes_local = [
                "Stable" if not phase_std_flag["Standing"] else "Slightly unstable",
                "Back tends to arch" if lumbar_compensation > 10 else "Smooth descent",
                "Some wobble observed" if phase_std_flag["Bottom"] else "Stable",
                "Left-right difference observed" if len(asym_flags) > 0 else "Stable, even rise",
            ]
        ...
        if client_lang_code == "ja":
            card_data = [
                ("可動域", mobility_score, "しゃがむ深さは十分に出ています。" if mobility_score >= 80 else "しゃがむ深さに、もう少し伸びしろがあります。"),
                ...
                ("姿勢のクセ", compensation_score, "腰・骨盤の反りは少なめです。" if compensation_score >= 80 else "しゃがみ立ちの中で、腰や骨盤が反りやすい傾向があります。"),
            ]
        else:
            card_data = [
                ("Mobility", mobility_score, "Good squat depth." if mobility_score >= 80 else "There is room to deepen the squat further."),
                ...
                ("Posture Habit", compensation_score, "Minimal lower-back/pelvis compensation." if compensation_score >= 80 else "The lower back and pelvis tend to arch during the squat."),
            ]
        ...
        band = Table(
            [[Paragraph(
                "あなたのスクワットチェック結果" if client_lang_code == "ja" else "Your Squat Check Results",
                c_title_style
            )],
                [Paragraph(
        ...
        if client_lang_code == "ja":
            ...
            elements_c.append(Paragraph(
                "スクワットの動き全体を、可動域・安定性・左右差・姿勢のクセの4つの視点でチェックしました。",
                c_lead_style
            ))
        else:
            elements_c.append(Paragraph(f"Overall: <b>{gauge_tier_label}</b>", c_score_headline_style))
            elements_c.append(Paragraph(
                "Your squat was checked across four areas: mobility, stability, symmetry, and posture habits.",
                c_lead_style
            ))
        ...
        elements_c.append(Paragraph(
            ("スクワットの動作を4つの場面に分けてみると、どこで体に負担がかかりやすいかが見えてきます。"
             if client_lang_code == "ja" else
             "Breaking the squat into four phases makes it easier to see where load tends to build up."),
            c_body_style
        ))
        ...
        detail_rows_c.append([
            JOINT_LABEL[client_lang_code]["pelvis_tilt"], f"{pelvic_compensation:.1f}°",
            ("やや大きめ" if pelvic_compensation > 10 else "少なめ") if client_lang_code == "ja" else ("Somewhat large" if pelvic_compensation > 10 else "Small")
        ])
        detail_rows_c.append([
            JOINT_LABEL[client_lang_code]["pelvis_rotation"], f"{pelvic_rotation_rom:.1f}°",
            ("やや大きめ" if pelvic_rotation_rom > 10 else "少なめ") if client_lang_code == "ja" else ("Somewhat large" if pelvic_rotation_rom > 10 else "Small")
        ])
        ...
        st.download_button(
            ...
            file_name="Squat_Client_Report.pdf",
            mime="application/pdf",
